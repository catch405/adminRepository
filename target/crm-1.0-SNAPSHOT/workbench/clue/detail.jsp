...
<script type="text/javascript">
	...
	$(function(){
		...
		//为关联市场活动模态窗口中的搜索框绑定事件,触发方式键盘敲击回车
		$("#aname").keydown(function (event) {
			...
			if (event.keyCode==13){
				...
				//展现完列表后,几记得将模态窗口默认的回车键清空页面的行为禁用
				return false;
			}
		...
		//为关联按钮绑定事件,执行关联表关系表添加操作
		$("#bunBtn").click(function () {
			...
			if ($xz.length==0){
				...
				$.ajax({
					...
					success:function (data) {
						...
						if (data.success){
							showActivityList();
							//清除搜索框的信息,复选框的勾干掉,清空activitySearchBody中的内容
							$("#aname").val("");
	...
	function unbund(id) {
		...
		//alert(id)此时的id经过后台处理,已经变成了关联表的id
		$.ajax({
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary"  id="bunBtn">关联</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<%--直接把convert需要展现的参数以在链接后追加参数的方式传递过去,就不用走后台(要考虑安全性和传递参数的值的长度--%>
